...
def status() -> dict:
    """NOTE: the message must reflect reality. A hardcoded "unset key" string here
    previously reported "Unset GEMINI_API_KEY" even when a key WAS configured —
    which sent the team chasing a bug that did not exist."""
    if available():
        note = ("Live — Gemini is handling extraction, translation and briefs.")
    else:
        note = ("No GEMINI_API_KEY found -> deterministic fallbacks are ACTIVE. "
                "The demo still runs, just less intelligently.")
    return {"configured": available(), "model": MODEL,
            "sdk": _sdk_available(), "note": note}
